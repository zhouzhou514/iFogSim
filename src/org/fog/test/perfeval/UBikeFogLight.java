    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if(CLOUD){
				...
				/*moduleMapping.addModuleToDevice("worker_reminder", "cloud"_s, numOfDepts*numOfSitesPerDept); // fixing all instances of the Connector module to the Cloud
				moduleMapping.addModuleToDevice("bikedata_collector", "cloud"_s, numOfDepts*numOfSitesPerDept); // fixing all instances of the Concentration Calculator module to the Cloud
*/				moduleMapping.addModuleToDevice("cloud_scheduler", "cloud"); // fixing all instances of the cloud prediction module to the Cloud
    ...
    private static Application createApplication(String appId, int userId){
         ...
         */
        application.addAppModule("cloud_scheduler", 1000, 1000);
        application.addAppModule("fog_predictor", 600, 1000); // adding module Client to the application model
        application.addAppModule("bikedata_collector", 50,100); // adding module Concentration Calculator to the application model
        application.addAppModule("worker_reminder", 100, 100); // adding module Connector to the application model
        /*
         * Connecting the application modules (vertices) in the application model (directed graph) with edges
         */
        if(UBIKE_TRANSMISSION_TIME==UBIKE_TRANSMISSION_TIME)
            application.addAppEdge("BIKE_USEAGE", "bikedata_collector", 20, 5, "BIKE_USEAGE", Tuple.UP, AppEdge.SENSOR); // adding edge from BIKE_USEAGE (sensor) to Client module carrying tuples of type BIKE_USEAGE
        else
            application.addAppEdge("BIKE_USEAGE", "bikedata_collector", 30, 5, "BIKE_USEAGE", Tuple.UP, AppEdge.SENSOR);

        application.addAppEdge("bikedata_collector", "fog_predictor", 20, 5, "SITE_STATE", Tuple.UP, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION
        application.addAppEdge("fog_predictor", "worker_reminder", 10, 10, "SCHEDULE_COMMAND", Tuple.DOWN, AppEdge.MODULE); // adding periodic edge (period=1000ms) from Connector to Client module carrying tuples of type GLOBAL_GAME_STATE
        ...
        application.addAppEdge("fog_predictor", "cloud_scheduler", 50, 20, "BLOCK_STATE", Tuple.UP, AppEdge.MODULE);  // adding edge from Client module to Display (actuator) carrying tuples of type GLOBAL_STATE_UPDATE
        application.addAppEdge("cloud_scheduler", "worker_reminder", 10, 10, "CROSS_BLOCK_SCHEDULE", Tuple.DOWN, AppEdge.MODULE);  // adding edge from Client module to Display (actuator) carrying tuples of type GLOBAL_STATE_UPDATE
